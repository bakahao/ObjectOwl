...
import com.example.objectowl.ml.VehicleModelV1;
...
public class VehicleModel {
    ...
    public static void classifyImage(Bitmap image, Context context, TextView result, TextView confidence, ImageView imageView) {
        try {
            ...
            VehicleModelV1 model = VehicleModelV1.newInstance(context);
            ...
            // Run the model
            VehicleModelV1.Outputs outputs = model.process(inputFeature0);
            ...
            String[] classes = {"Car", "Bicycle", "Motorcycle", "Bus", "Train",
                    ...
                    "Ambulance", "Fire truck", "Helicopter", "Van", "Excavator",
                     "Forklift", "Garbage truck"};
    ...
    private static void displayConfidences(Context context, String recognizedAnimal, TextView confidence) {
        try {
            ...
            InputStream fis = assetManager.open("vehicle_des.xlsx");
            ...
            for (Row row : sheet) {
                String vehicleName = row.getCell(0).getStringCellValue(); // Assuming animal names are in the first column
                ...
                if (vehicleName.equalsIgnoreCase(recognizedAnimal)) {
